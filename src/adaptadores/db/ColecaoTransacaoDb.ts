...
export default class ColecaoTransacaoDb implements ColecaoTransacao {
    ...
    adicionar(transacao: Transacao): Promise<void> {
        return conexao.table('transacoes').insert(
            this._praTabela(transacao)
        )
    }
    atualizar(transacao: Transacao): Promise<void> {
        return conexao.table('transacoes')
    ...
    async buscarPorId(idUsuario: string, id: string): Promise<Transacao | null> {
       ...
       const transacoes = await conexao.table('transacoes').where({id,usuario_id: idUsuario})
    ...
    async bucarPorMes(idUsuario: string, ano: number, mes: number): Promise<Transacao | null> {
        ...
        return this._daTabela(transacoes)
    }
